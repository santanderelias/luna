...
function addRecurringIncome() {
    ...
    const newIndex = settings.recurringIncomes.length - 1;
    toggleRecurringIncomeEditMode(newIndex, true); // Immediately enter edit mode for the new item
}

function deleteRecurringIncome(index) {
    settings.recurringIncomes.splice(index, 1);
    saveRecurringIncomeSettings(); // Save and refresh after deleting
}
...
function addFixedExpense() {
    ...
    if (description && amount > 0) {
        ...
        saveFixedExpensesSettings(); // Save and refresh after adding
    ...
    showConfirmationModal('Are you sure you want to delete this fixed expense?', [
        {
            ...
            callback: () => {
                ...
                saveFixedExpensesSettings(); // Save and refresh after deleting
            }
        }
    ]);
}
...
// Event listener for accordion collapse to cancel edits
document.addEventListener('DOMContentLoaded', () => {
    const collapseRecurringIncome = document.getElementById('collapseRecurringIncome');
    if (collapseRecurringIncome) {
        collapseRecurringIncome.addEventListener('hide.bs.collapse', () => {
            settings.recurringIncomes.forEach((_, index) => {
                const editDiv = document.getElementById(`recurring-income-edit-${index}`);
                if (editDiv && editDiv.style.display !== 'none') {
                    toggleRecurringIncomeEditMode(index, false); // Cancel edit mode
                }
            });
        });
    }
